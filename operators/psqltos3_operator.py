...
class downloadFromS3Operator(BaseOperator):
    ...
    def __init__(self, s3_conn_id: str, s3_bucket: str, s3_key: str, local_path: str, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.s3_conn_id = s3_conn_id
        self.s3_bucket = s3_bucket
        self.s3_key = s3_key
...
class psqlToS3Operator(BaseOperator):
    ...
    @apply_defaults
    def __init__(self, postgres_conn_id: str, s3_conn_id: str, s3_bucket: str, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.postgres_conn_id = postgres_conn_id
        self.s3_conn_id = s3_conn_id
        self.s3_bucket = s3_bucket
...
class insertRedshiftFromS3Operator(BaseOperator):
    ...
    def __init__(self, redshift_conn_id: str, s3_conn_id: str, s3_bucket: str, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.redshift_conn_id = redshift_conn_id
        self.s3_conn_id = s3_conn_id
        self.s3_bucket = s3_bucket
    ...
    def execute(self, context):
        ...
        for table in table_list:
            ...
            # if table in ["avatar", "bidata", "space_editor", "lti_platform_record", "section", "assignment_result"]:
            #    continue
